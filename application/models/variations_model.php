...
*/
  ...
  */
  ...
  public function get_diff_stats($timeStamp){
    ...
    //on g.gene=pno.gene
    //left join
    //(select gene, count(*) as old_num_lp from oldTable where pathogenicity='Likely pathogenic' group by gene) lpno on g.gene=lpno.gene left join (select gene, count(*) as old_num_us from oldTable where pathogenicity='Unknown significance' group by gene) usno on g.gene=usno.gene left join (select gene, count(*) as old_num_lb from oldTable where pathogenicity='Likely benign' group by gene) lbno on g.gene=lbno.gene left join (select gene, count(*) as old_num_b from oldTable where pathogenicity='Benign' group by gene) bno on g.gene=bno.gene left join (select gene, count(*) as old_num_bs from oldTable where pathogenicity='Benign*' group by gene) bsno on g.gene=bsno.gene left join (select gene, count(*) as changed from commonVariants where old_path != new_path group by gene) c on g.gene=c.gene left join (select gene, count(*) as unchanged from commonVariants where old_path = new_path group by gene) u on g.gene=u.gene left join (select gene, count(*) as num_p_to_lp from commonVariants where old_path='Pathogenic' and new_path='Likely pathogenic' group by gene) t1 on g.gene=t1.gene left join (select gene, count(*) as num_p_to_us from commonVariants where old_path='Pathogenic' and new_path='Unknown significance' group by gene) t2 on g.gene = t2.gene left join (select gene, count(*) as num_p_to_lb from commonVariants where old_path='Pathogenic' and new_path='Likely benign' group by gene) t3 on g.gene = t3.gene left join (select gene, count(*) as num_p_to_b from commonVariants where old_path='Pathogenic' and new_path='Benign' group by gene) t4 on g.gene = t4.gene left join (select gene, count(*) as num_p_to_bs from commonVariants where old_path='Pathogenic' and new_path='Benign*' group by gene) t5 on g.gene = t5.gene left join (select gene, count(*) as num_lp_to_p from commonVariants where old_path='Likely pathogenic' and new_path='Pathogenic' group by gene) t6 on g.gene = t6.gene left join (select gene, count(*) as num_lp_to_us from commonVariants where old_path='Likely pathogenic' and new_path='Unknown significance' group by gene) t7 on g.gene = t7.gene left join (select gene, count(*) as num_lp_to_lb from commonVariants where old_path='Likely pathogenic' and new_path='Likely benign' group by gene) t8 on g.gene = t8.gene left join (select gene, count(*) as num_lp_to_b from commonVariants where old_path='Likely pathogenic' and new_path='Benign' group by gene) t9 on g.gene = t9.gene left join (select gene, count(*) as num_lp_to_bs from commonVariants where old_path='Likely pathogenic' and new_path='Benign*' group by gene) t10 on g.gene = t10.gene left join (select gene, count(*) as num_us_to_p from commonVariants where old_path='Unknown significance' and new_path='Pathogenic' group by gene) t11 on g.gene = t11.gene left join (select gene, count(*) as num_us_to_lp from commonVariants where old_path='Unknown significance' and new_path='Likely pathogenic' group by gene) t12 on g.gene = t12.gene left join (select gene, count(*) as num_us_to_lb from commonVariants where old_path='Unknown significance' and new_path='Likely benign' group by gene) t13 on g.gene = t13.gene left join (select gene, count(*) as num_us_to_b from commonVariants where old_path='Unknown significance' and new_path='Benign' group by gene) t14 on g.gene = t14.gene left join (select gene, count(*) as num_us_to_bs from commonVariants where old_path='Unknown significance' and new_path='Benign*' group by gene) t15 on g.gene = t15.gene left join (select gene, count(*) as num_lb_to_p from commonVariants where old_path='Likely benign' and new_path='Pathogenic' group by gene) t16 on g.gene = t16.gene left join (select gene, count(*) as num_lb_to_lp from commonVariants where old_path='Likely benign' and new_path='Likely pathogenic' group by gene) t17 on g.gene = t17.gene left join (select gene, count(*) as num_lb_to_us from commonVariants where old_path='Likely benign' and new_path='Unknown significance' group by gene) t18 on g.gene = t18.gene left join (select gene, count(*) as num_lb_to_b from commonVariants where old_path='Likely benign' and new_path='Benign' group by gene) t19 on g.gene = t19.gene left join (select gene, count(*) as num_lb_to_bs from commonVariants where old_path='Likely benign' and new_path='Benign*' group by gene) t20 on g.gene = t20.gene left join (select gene, count(*) as num_b_to_p from commonVariants where old_path='Benign' and new_path='Pathogenic' group by gene) t21 on g.gene = t21.gene left join (select gene, count(*) as num_b_to_lp from commonVariants where old_path='Benign' and new_path='Likely pathogenic' group by gene) t22 on g.gene = t22.gene left join (select gene, count(*) as num_b_to_us from commonVariants where old_path='Benign' and new_path='Unknown significance' group by gene) t23 on g.gene = t23.gene left join (select gene, count(*) as num_b_to_lb from commonVariants where old_path='Benign' and new_path='Likely benign' group by gene) t24 on g.gene = t24.gene left join (select gene, count(*) as num_b_to_bs from commonVariants where old_path='Benign' and new_path='Benign*' group by gene) t25 on g.gene = t25.gene left join (select gene, count(*) as num_bs_to_p from commonVariants where old_path='Benign*' and new_path='Pathogenic' group by gene) t26 on g.gene = t26.gene left join (select gene, count(*) as num_bs_to_lp from commonVariants where old_path='Benign*' and new_path='Likely pathogenic' group by gene) t27 on g.gene = t27.gene left join (select gene, count(*) as num_bs_to_us from commonVariants where old_path='Benign*' and new_path='Unknown significance' group by gene) t28 on g.gene = t28.gene left join (select gene, count(*) as num_bs_to_lb from commonVariants where old_path='Benign*' and new_path='Likely benign' group by gene) t29 on g.gene = t29.gene left join (select gene, count(*) as num_bs_to_b from commonVariants where old_path='Benign*' and new_path='Benign' group by gene) t30 on g.gene = t30.gene";
